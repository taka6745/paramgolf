...
| TOKEN_FIREHOSE_3way | - | normuon + gated_attention + asymmetric_skip_init (= STACK_TRUE_WINNERS_3WAY, fresh seed) | - | **★ n=3 confirmed-win mean=1.40287** | S2 seed42=1.4032 + seed1337=1.4027 + STRETCH_HymbaMamba2_seed1337=**1.4073** (also 3-way same env) → mean (3 seed42-class results) =**1.40287** (Δ-0.011 vs baseline; **BEST 3-way confirmed**, very stable) | **1.40287** ★ (cheap J n=3) | J | 20260408T1707Z |
...
| STACK_LEGAL_TTT_NGRAM_BACKOFF | - | gated_attention + LEGAL_TTT + NGRAM_BACKOFF (cross-layer L04+L09) | **yes (world-novel combination)** | **n=1 CATASTROPHIC FAIL** | S2 seed42=**1.4567** vs baseline 1.4137 = **+0.043 ABOVE** (worst stack result yet); vs LEGAL_TTT champion 1.3711 = **+0.0856 WORSE**. CONFIRMED: even cross-layer (n-gram residual logit, not model weights) breaks LEGAL_TTT. **CHAMPION IS UNIQUE BRITTLE LOCAL MINIMUM**. seed1337 in flight | +0.0856 vs champion | J | 20260408T1822Z |
| STACK_BACKOFF_GATED | - | gated_attention + NGRAM_BACKOFF (cross-layer L04+L09 WITHOUT LEGAL_TTT) | **yes (world-novel combination)** | **★ n=1 CONTROL TEST CONFIRMED FAIL** | S2 seed42=**1.4162** vs baseline 1.4137 = +0.0025 (FAIL); vs gated alone 1.4094 = +0.0068 WORSE; vs backoff alone 1.4122 = +0.004 WORSE. **CROSS-LAYER COMPOSITIONALITY IS BROKEN GENERALLY**, not just for LEGAL_TTT. Two confirmed wins COMBINED < either alone | +0.0068 vs gated alone (n=1) | I | 20260408T1827Z |
...
| STACK_GATED_LEGAL_TTT_5STEPS | - | bare champion + LEGAL_TTT_STEPS=5 (vs default 3) — direct hyperparameter tuning | **comp-novel (PhD-defensible)** | **C90 1850Z queued** | hypothesis: champion 1.3711 uses LEGAL_TTT_STEPS=3 default; bumping to 5 gives ~67% more eval-time SGD steps. Could push below 1.3666 bear-fruit threshold if eval is under-converged. n=2 for Pod F | pending | F | 20260408T1850Z |
...
| G2_gpu_idle_streak | 20260408T1852Z | 0 idle streaks (all 8 pods 92-100%); H picked up STACK_GATED_LEGAL_TTT_LN_SCALE_seed42 (1820Z ship LIVE); 3 NIGHT_MODE shots in flight | 0 streaks >5s util<80% | PASS | 0 |
