#!/usr/bin/env python3
"""
Parameter Golf MLX v2 - Table-stakes improvements ported from SOTA entries.

Changes from baseline:
- 11 layers (was 9), 3x MLP (was 2x)
- LeakyReLU(0.5)^2 activation (was ReLU^2)
- SmearGate (bigram position blending)
- BigramHash embeddings (3072 buckets, 112 dim)
...
class Hyperparameters:
    ...
    rope_dims: int = int(os.environ.get("ROPE_DIMS", 16))  # partial RoPE
...
class BigramHashEmbedding(nn.Module):
    """Hash-based bigram embeddings for context-aware token representations."""
    def __init__(self, bigram_vocab_size: int, bigram_dim: int, model_dim: int):
        super().__init__()
        self.bigram_vocab_size = bigram_vocab_size
        self.embed = nn.Embedding(bigram_vocab_size, bigram_dim)
        # Zero-init so bigrams start as no-op
    ...
    def __call__(self, token_ids: mx.array) -> mx.array:
        ...
        # Hash: XOR of scaled token pairs
        hashed = mx.zeros_like(t)
        # First position gets the sentinel bucket
        first_col = mx.full(t[:, :1].shape, mod, dtype=mx.int32)
        rest = (36313 * t[:, 1:] ^ 27191 * t[:, :-1]) % mod
        hashed = mx.concatenate([first_col, rest], axis=1)
        h = self.embed(hashed)
        if self.proj is not None:
            h = self.proj(h)
...
class PartialRoPE(nn.Module):
    """RoPE applied to only the first `rope_dims` dimensions of each head."""
    def __init__(self, head_dim: int, rope_dims: int, base: float = 10000.0):
        super().__init__()
        self.rope_dims = rope_dims
        self.head_dim = head_dim
        # Only create frequencies for the rope_dims subset
        self.rope = nn.RoPE(rope_dims, traditional=False, base=base)

    def __call__(self, x: mx.array) -> mx.array:
        if self.rope_dims >= self.head_dim:
            return self.rope(x)
        # Split: apply RoPE to first rope_dims, pass through the rest
        x_rope = x[..., :self.rope_dims]
        x_pass = x[..., self.rope_dims:]
        x_rope = self.rope(x_rope)
        return mx.concatenate([x_rope, x_pass], axis=-1)


class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float,
                 qk_gain_init: float, rope_dims: int, use_xsa: bool = True):
        super().__init__()
    ...
    def _xsa(self, y: mx.array, v: mx.array) -> mx.array:
        """Exclusive Self-Attention: remove self-value component from attention output."""
        B, H, T, D = y.shape
        Hkv = v.shape[1]
        group = H // Hkv
        # Reshape y for GQA groups: [B, Hkv, group, T, D]
        y_g = y.reshape(B, Hkv, group, T, D)
        # Normalize v and broadcast: [B, Hkv, 1, T, D]
        v_t = v.transpose(0, 1, 2, 3)  # [B, Hkv, T, D]
        v_norm = v_t / (mx.sqrt(mx.sum(v_t * v_t, axis=-1, keepdims=True)) + 1e-8)
        vn = mx.expand_dims(v_norm, axis=2)  # [B, Hkv, 1, T, D]
        # Project out self-value direction
...
class MLP(nn.Module):
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        h = self.fc(x)
        h = mx.where(h >= 0, h, 0.5 * h)  # leaky_relu with slope 0.5
        return self.proj(h * h)  # square


class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 rope_base: float, qk_gain_init: float, rope_dims: int,
                 layer_idx: int, use_xsa: bool = True, ln_scale: bool = True):
        super().__init__()
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int,
                 ...
                 tied_embed_init_std: float, qk_gain_init: float, rope_dims: int,
                 bigram_vocab_size: int, bigram_dim: int):
        super().__init__()
        ...
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                  rope_dims=rope_dims, layer_idx=i, use_xsa=True, ln_scale=True)
            for i in range(num_layers)
        ]
        self.final_norm = RMSNormNoWeight()

        # Initialize weights: zero-init projection layers (same as baseline)
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        # Add bigram embeddings before normalization
...
class SplitOptimizers:
    def __init__(self, model: GPT, args: Hyperparameters):
        ...
        # Include bigram and smear params in scalar optimizer
        for k in params:
            if (k.startswith("bigram.") or k.startswith("smear.")) and k not in self.scalar_keys:
                if k not in [self.embed_key] and k not in self.matrix_keys:
...
def validate_dataset_tokenizer_pair(data_path: str, tokenizer_path: str) -> tuple[str, int, int | None]:
    ...
    tokenizer_entry = (
        next((x for x in manifest.get("tokenizers", []) if x.get("name") == tokenizer_name), None) if tokenizer_name else None
    )
...
def main() -> None:
    ...
    # EMA state: deep copy of initial parameters (keep as mx arrays)
    ema_state = {k: v * 1 for k, v in tree_flatten(model.state)}
    ...
    log(f"model_params:{n_params} vocab_size:{args.vocab_size} layers:{args.num_layers} dim:{args.model_dim} "
        f"heads:{args.num_heads} kv_heads:{args.num_kv_heads} seq_len:{args.train_seq_len} "
        f"mlp_mult:{args.mlp_mult} rope_dims:{args.rope_dims} bigram:{args.bigram_vocab_size}x{args.bigram_dim}")
    ...
    log(f"optimizer:muon+adam muon_matrix_params:{len(opt.matrix_keys)} scalar_params:{len(opt.scalar_keys)} "
        ...
        f"muon_momentum:{args.muon_momentum} muon_steps:{args.muon_backend_steps} weight_decay:{args.weight_decay}")
    log(f"ema_decay:{args.ema_decay}")
    ...
    while True:
        ...
        # EMA update (only every 10 steps to save memory/compute)
        if step % 10 == 0:
