...
| STACK_GATED_LEGAL_TTT_LN_SCALE | - | gated_attention + LEGAL_TTT + LN_SCALE (smallest perturbation to champion) | **yes (world-novel combination)** | **queued for H (post current)** | hypothesis: ABSOLUTE MINIMUM perturbation test — add ONLY LN_SCALE (1/√(layer+1) RMSNorm scaling, single scalar mod, no new params) | pending | H | 20260408T1820Z |
| STACK_GATED_LEGAL_TTT_5STEPS | - | bare champion + LEGAL_TTT_STEPS=5 (vs default 3) | comp-novel | **n=1 FAIL — default STEPS=3 is optimum** | S2 seed42=**1.4142** vs baseline 1.4137 = +0.0005 (neutral vs baseline), vs champion 1.3711 = **+0.0431 WORSE**. More SGD steps HURTS — LEGAL_TTT is NOT under-converged at default. seed1337 pending | +0.0431 vs champion | F | 20260408T1927Z |
...
| G2_gpu_idle_streak | 20260408T1947Z | 0 idle streaks (all 8 pods 73-100%); H=LN_SCALE 56min (still running, eval very slow); F=5STEPS_seed1337 20min; J=LR2X_seed42 12min | 0 streaks >5s util<80% | PASS | 0 |
