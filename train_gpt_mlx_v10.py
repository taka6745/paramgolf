...
class GPT(nn.Module):
    ...
    def _add_ngram_bias(self, logits: mx.array, prev_tokens: mx.array,
                        ...
                        prev3_tokens: mx.array | None = None) -> mx.array:
        """Add bigram + trigram + 4-gram log-probability biases to model logits."""
        ...
        if self._fourgram_logprobs is not None and prev2_tokens is not None and prev3_tokens is not None:
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        # Cross-entropy with bigram + trigram + 4-gram logit biases.
        x = self(input_ids).reshape(-1, self.tok_emb.weight.shape[1])
        y = target_ids.reshape(-1)
        input_flat = input_ids.reshape(-1)
        prev_tokens = input_flat  # prev1 (t-1)
        ...
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            ...
            logits = self._add_ngram_bias(self.softcap(logits_proj), prev_tokens, prev2_tokens, prev3_tokens)
        ...
        for s in range(0, n, self.logit_chunk_tokens):
            ...
            logits = self._add_ngram_bias(self.softcap(logits_proj), prev_tokens[s:e], prev2_tokens[s:e], prev3_tokens[s:e])
