...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        # Bigram logprob bias: shape [vocab_size, vocab_size], bigram_logprobs[prev][next]
    ...
    def _add_bigram_bias(self, logits: mx.array, prev_tokens: mx.array) -> mx.array:
        """Add bigram log-probability bias to model logits.
        prev_tokens: [N] flat array of previous token ids.
        logits: [N, vocab_size] model logits."""
        if self._bigram_logprobs is None:
            return logits
        # Look up bigram logprobs for each position's previous token
        # bigram_bias[i] = bigram_logprobs[prev_tokens[i], :] — shape [N, vocab_size]
        bigram_bias = self._bigram_logprobs[prev_tokens]  # [N, vocab_size]
        return logits + self._bigram_weight * bigram_bias.astype(logits.dtype)

    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        # Cross-entropy over flattened tokens with optional bigram logit bias.
        x = self(input_ids).reshape(-1, self.tok_emb.weight.shape[1])
        y = target_ids.reshape(-1)
        # Previous tokens for bigram lookup (input_ids shifted)
        prev_tokens = input_ids.reshape(-1)
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            logits_proj = x @ self.tok_emb.weight.astype(x.dtype).T
            logits = self._add_bigram_bias(self.softcap(logits_proj), prev_tokens)
        ...
        for s in range(0, n, self.logit_chunk_tokens):
            ...
            logits = self._add_bigram_bias(self.softcap(logits_proj), prev_tokens[s:e])
