...
class MLP(nn.Module):
    ...
    def __call__(self, x: mx.array) -> mx.array:
        h = self.fc(x)
        h = mx.where(h >= 0, h, 0.5 * h)  # LeakyReLU(0.5)
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 ...
                 qk_gain_init: float, byte_weights: mx.array | None = None):
        super().__init__()
        if logit_softcap <= 0.0:
            raise ValueError(f"logit_softcap must be positive, got {logit_softcap}")
        self.logit_chunk_tokens = logit_chunk_tokens
        self.logit_softcap = logit_softcap
        # byte_weights[token_id] = bytes represented by that token (for frequency-weighted loss)
        self._byte_weights = byte_weights  # not a parameter, just a lookup
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        # Cross-entropy over flattened tokens, optionally weighted by bytes-per-token.
        # Byte-weighted loss aligns training objective with the bpb metric:
        # tokens covering more bytes contribute proportionally more to loss.
        ...
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            ...
            if self._byte_weights is not None:
                w = self._byte_weights[y].astype(mx.float32)
                return mx.sum(per_token_loss * w) / mx.sum(w)
        ...
        for s in range(0, n, self.logit_chunk_tokens):
            ...
            logits = self.softcap(logits_proj)
            per_token_loss = nn.losses.cross_entropy(logits.astype(mx.float32), y[s:e], reduction="none")
            if self._byte_weights is not None:
                w = self._byte_weights[y[s:e]].astype(mx.float32)
                loss_sum = loss_sum + mx.sum(per_token_loss * w)
                weight_sum = weight_sum + mx.sum(w)
            else:
                loss_sum = loss_sum + mx.sum(per_token_loss)
...
def main() -> None:
    ...
    # Build byte-weight lookup for frequency-weighted loss.
    # Each token's weight = max(bytes_it_represents, 1) so the loss aligns with bpb metric.
    byte_weights_np = np.maximum(base_bytes_lut.astype(np.float32), 1.0)
    byte_weights = mx.array(byte_weights_np, dtype=mx.float32)
    log(f"byte_weighted_loss:enabled mean_weight:{float(byte_weights_np.mean()):.2f} max_weight:{float(byte_weights_np.max()):.0f}")
