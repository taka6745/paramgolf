...
| L02_data | 2 | L02_mdl_compressible_first (BEC_REVERSE=1, MDL anti-curriculum, world-novel-candidate) | **★ n=2 seed42 mean=1.4101** | seed42=1.4093 + seed42-rerun=**1.4109** → mean=**1.4101** vs baseline 1.4137 = -0.0036 (real win, ~0.001 within-seed variance); proper seed1337 in flight on Pod E | **1.4101** ★ (cheap E n=2) | E | 20260408T1807Z |
...
| STACK_GATED_LEGAL_TTT_LN_SCALE | - | gated_attention + LEGAL_TTT + LN_SCALE (smallest perturbation to champion) | **yes (world-novel combination)** | **C90 1820Z queued** | hypothesis: ABSOLUTE MINIMUM perturbation test — add ONLY LN_SCALE (1/√(layer+1) RMSNorm scaling, single scalar mod, no new params). If even this fails, LEGAL_TTT champion is brittle to LITERALLY anything beyond gated_attention. n=2 for Pod H | pending | H | 20260408T1820Z |
...
| G2_gpu_idle_streak | 20260408T1832Z | 0 idle streaks (all 8 pods 79-100%); 2 NIGHT_MODE shots in flight (I=BACKOFF_GATED_seed1337 confirm, J=LEGAL_TTT_NGRAM_BACKOFF_seed1337 confirm) | 0 streaks >5s util<80% | PASS | 0 |
