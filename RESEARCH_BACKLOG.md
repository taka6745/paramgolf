...
| 11 | KER_cudagraph_persistent_kernel_fusion | C30 1241Z — Deep Kernel Fusion arXiv:2602.11808 (Jan 2025) + cudagraph_mark_step_begin pattern | wrap forward+backward+opt.step as single persistent CUDA graph with unified memory; persistent kernel occupancy=100% on MLP blocks (SwiGLU fc→act→fc fused); eliminates launch overhead between blocks | -10 to -18% step time → +95-140 extra steps → -0.012 to -0.025 BPB indirect | **world-novel-candidate** | 280 | 20260408T1241Z |
| 12 | SPD_inductor_compile_mode_reduce_overhead | C30 1241Z — torch.compile state-of-art (ezyang Aug 2025) + HF guide | torch.compile w/ mode="reduce-overhead" on attention+MLP blocks (regional, not full model); Inductor generates fused Triton kernels for matmul+act+bias; skip dynamic shapes to avoid recompile | -8 to -15% step time → -0.008 to -0.015 BPB indirect | comp-novel (reduce-overhead mode known but unused in comp PRs) | 60 | 20260408T1241Z |
| 13 | SPD_kvquant_lowrank_cache_training | C30 1241Z — KVQuant NeurIPS 2024 + Expected Attention Oct 2025 + NVIDIA NVFP4 | store KV cache in int8 (or NVFP4) DURING TRAINING forward pass; learned per-layer affine dequant; 60-75% KV bandwidth saving frees budget for larger batch | -6 to -12% attn block time → -0.006 to -0.014 BPB indirect | **world-novel-candidate** (KVQuant is inference-only; training-time KV quant w/ learned scales is novel) | 130 | 20260408T1241Z |
...
| 6 | ATT_differential_attention_qk_partition | C30 1241Z — Microsoft ICLR 2025 arXiv:2410.05258 | split Q/K into TWO independent attention maps and SUBTRACT softmax(Q₂K₂) from softmax(Q₁K₁) with learnable per-head λ ∈ [0.1,1.0]; cancels softmax flattening on byte-vocab | -0.008 to -0.015 train_loss | comp-novel (published Oct 2024, 0 byte-LM application) | 90 | 20260408T1241Z |
| 7 | ATT_xattention_block_sparse_antidiag | C30 1241Z — MIT Han Lab ICML 2025 (XAttention) | block-sparse attention with antidiagonal scoring; partition seq into blocks size 64 (matching head_dim), only compute within-block + k antidiag neighbors | -15 to -25% attn forward time → -0.002 to -0.006 train_loss | comp-novel (XAttention May 2025, no comp PRs use it) | 120 | 20260408T1241Z |
| 8 | ATT_linear_gated_hyena_hybrid | C30 1241Z — RWKV-7 "Goose" arXiv:2504.21463 + Hyena-style gating | replace 1-2 middle attn layers with single fast linear-attention layer w/ per-head gating + learnable K_decay; selective hybrid (keep dense attn on L01/L02/L_final) | -8 to -12% step time → -0.003 to -0.008 train_loss | **world-novel-candidate** (per-head decay gates in selective linear-attn replacement is custom, distinct from Hymba SSM) | 85 | 20260408T1241Z |
...
| 7 | FFN_spark_statistical_topk_sparse | C30 1349Z — Spark Transformer arXiv:2506.06644 NeurIPS 2025 | replace ReLU² with statistical top-k sparsity: fit Gaussian to activation scores, threshold for top-8% active. STE backward. Forced specialization | -0.008 to -0.015 BPB | **world-novel-candidate** (Spark byte-LM application = 0 hits) | 120 | 20260408T1349Z |
| 8 | FFN_polynomial_cubic_mixed_gate | C30 1349Z — arXiv:1906.09529 polynomial activations + custom byte-LM synthesis | replace ReLU² with learnable degree-3 polynomial: σ(x) = a·x³ + b·x² + c·x + d, per-layer params init to match ReLU². Adapt nonlinearity per layer | -0.005 to -0.012 BPB | **world-novel-candidate** (polynomial in FFN gate for byte-LM = 0 hits) | 45 | 20260408T1349Z |
| 9 | FFN_contextual_countdown_sparse_down_projection | C30 1349Z — COUNTDOWN arXiv:2505.17701 May 2025 | tiny 2-layer NN gate predicts which hidden_dim dims are unnecessary per token+layer; zero them before down-projection. Skip rare-context computations | -0.006 to -0.014 BPB + ~0 step overhead | **world-novel-candidate** (COUNTDOWN byte-LM application = 0 hits) | 85 | 20260408T1349Z |
...
| 5 | NRM_dynamic_tanh_per_layer | C30 1349Z — Meta FAIR arXiv:2503.10622 DyT (Dec 2024) | replace RMSNorm with element-wise tanh: γ_ℓ·tanh(α_ℓ·x) + β_ℓ. Eliminates statistics gather (~20% norm op savings), tanh saturation stabilizes byte-scale gradient | -0.008 to -0.015 BPB | **world-novel-candidate** (DyT byte-LM application = 0 hits) | 40 | 20260408T1349Z |
| 6 | NRM_sandwich_peri_dual_rmsn | C30 1349Z — HybridNorm 2025 + Peri-LN (Gemma/OLMo 2024) | dual RMSNorm pre+post each sublayer (attn AND MLP); current is pre-norm only. Self-regularizer constraining both pre/post sublayer distributions | -0.006 to -0.012 BPB | comp-novel-with-0-PR-hits | 30 | 20260408T1349Z |
| 7 | NRM_rmsnorm_temperature_adaptive_decay | C30 1349Z — Bolmo arXiv:2512.15586 + temp scaling synthesis | learn per-layer RMSNorm exponent α_ℓ ∈ [0.5, 2.0] init 1.0; scale = 1/rms(x)^α_ℓ. Distinct from LN_SCALE_MARKER (fixed 1/√(ℓ+1)); learned exponent | -0.005 to -0.011 BPB | **world-novel-candidate** (learnable RMSNorm exponent = 0 hits) | 35 | 20260408T1349Z |
