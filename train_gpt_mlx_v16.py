...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        # SVD-based embedding init: use top-k singular vectors of bigram probability matrix
        svd_path = Path('data/bigram_svd_U.npy')
        if svd_path.exists():
            svd_U = np.load(str(svd_path))  # [1024, 512]
            svd_S = np.load(str(Path('data/bigram_svd_S.npy')))  # [512]
            # Scale by sqrt of singular values for balanced init
            init_emb = svd_U * np.sqrt(svd_S)[None, :] * tied_embed_init_std / np.sqrt(svd_S[0])
            self.tok_emb.weight = mx.array(init_emb[:vocab_size, :dim], dtype=COMPUTE_DTYPE)
        else:
            self.tok_emb.weight = (
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        # Cross-entropy with n-gram logit biases (2-5 gram).
