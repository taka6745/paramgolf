...
| STACK_LEGAL_TTT_NGRAM_BACKOFF | - | gated_attention + LEGAL_TTT + NGRAM_BACKOFF (cross-layer L04+L09) | **yes (world-novel combination)** | **in-flight Pod J (started 1723Z, lands ~1750Z)** | hypothesis: cross-LAYER stack (L04 model-side LEGAL_TTT champion + L09 n-gram-side BACKOFF bias) is ORTHOGONAL. **LAST hope tonight to compose with LEGAL_TTT** | pending | J | 20260408T1727Z |
| STACK_BACKOFF_GATED | - | gated_attention + NGRAM_BACKOFF (cross-layer L04+L09 WITHOUT LEGAL_TTT) | **yes (world-novel combination)** | **C90 1750Z queued** | hypothesis: control test for cross-layer compositionality WITHOUT the brittle LEGAL_TTT ingredient. Both gated_attention (1.4094) + NGRAM_BACKOFF (1.4122) confirmed wins. If this composes well to ~1.405, validates that LEGAL_TTT is the brittle component, NOT cross-layer stacking. n=2 for Pod I | pending | I | 20260408T1750Z |
...
| G2_gpu_idle_streak | 20260408T1813Z | 0 idle streaks (all 8 pods 79-100%); 2 NIGHT_MODE shots (I=BACKOFF_GATED 19min, J=LEGAL_TTT_NGRAM_BACKOFF 50min — eval extends past wallclock) | 0 streaks >5s util<80% | PASS | 0 |
