...
| STACK_GATED_LEGAL_TTT_LR2X | - | bare champion + LEGAL_TTT_LR=0.002 (vs default 0.001) — 2× learning rate | **comp-novel (PhD-defensible)** | **in-flight Pod J (started 1934Z)** | hypothesis: complement to 5STEPS test. Doubles per-step LR; tests whether step COUNT or step SIZE matters | pending | J | 20260408T1920Z |
| STACK_GATED_LEGAL_TTT_LR_HALF | - | bare champion + LEGAL_TTT_LR=0.0005 (vs default 0.001) — half learning rate | **comp-novel (PhD-defensible)** | **C90 1950Z queued** | hypothesis: maps LEGAL_TTT LR landscape completely (0.0005/0.001/0.002). If default is optimum, LEGAL_TTT is converged; if lower wins, was over-stepping; if higher (LR2X) wins, was under-stepping. n=2 for Pod B | pending | B | 20260408T1950Z |
...
| G2_gpu_idle_streak | 20260408T2027Z | Pod E briefly at 48% util (1st yellow flag, needs 2 consecutive to escalate); 7 pods 100% GPU; 3 NIGHT_MODE LEGAL_TTT sweep still in flight (30min/39min/52min) | 0 streaks >5s util<80% | PASS | 0 |
