...
| STACK_LEGAL_TTT_NGRAM_BACKOFF | - | gated_attention + LEGAL_TTT + NGRAM_BACKOFF (cross-layer L04+L09) | **yes (world-novel combination)** | **in-flight Pod J (started 1723Z)** | hypothesis: cross-LAYER stack (L04 model-side LEGAL_TTT champion + L09 n-gram-side BACKOFF bias) is ORTHOGONAL. **LAST hope tonight to compose with LEGAL_TTT** after gated+norm_pct stacks all failed. If this also fails, LEGAL_TTT is brittle to ALL additions period | pending | J | 20260408T1727Z |
...
| G2_gpu_idle_streak | 20260408T1732Z | 0 idle streaks (all 8 pods 94-100%); B/E/G/J on NIGHT_MODE seed1337 confirms; J=STACK_LEGAL_TTT_NGRAM_BACKOFF_seed42 (cross-layer test) | 0 streaks >5s util<80% | PASS | 0 |
