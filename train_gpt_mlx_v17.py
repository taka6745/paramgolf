...
class SmearGate(nn.Module):
    """Learned gate that blends each token embedding with the previous token."""
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        # Shift right by 1: pad left with zeros, drop last position
...
class MLP(nn.Module):
    ...
    def __call__(self, x: mx.array) -> mx.array:
        # LeakyReLU(0.5)^2: preserves negative gradient flow, no dead neurons
        h = self.fc(x)
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 ...
                 qk_gain_init: float, bigram_logprobs: mx.array | None = None, bigram_weight: float = 0.3,
                 trigram_logprobs: mx.array | None = None, trigram_weight: float = 0.15,
                 trigram_buckets: int = 65536,
                 fourgram_logprobs: mx.array | None = None, fourgram_weight: float = 0.1,
                 fourgram_buckets: int = 65536,
                 fivegram_logprobs: mx.array | None = None, fivegram_weight: float = 0.08,
                 fivegram_buckets: int = 65536):
        super().__init__()
        if logit_softcap <= 0.0:
            raise ValueError(f"logit_softcap must be positive, got {logit_softcap}")
        self.logit_chunk_tokens = logit_chunk_tokens
        self.logit_softcap = logit_softcap
        self._bigram_logprobs = bigram_logprobs
        self._bigram_weight = bigram_weight
        self._trigram_logprobs = trigram_logprobs
        self._trigram_weight = trigram_weight
        self._trigram_buckets = trigram_buckets
        self._fourgram_logprobs = fourgram_logprobs
        self._fourgram_weight = fourgram_weight
        self._fourgram_buckets = fourgram_buckets
        self._fivegram_logprobs = fivegram_logprobs
        self._fivegram_weight = fivegram_weight
        self._fivegram_buckets = fivegram_buckets
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        for i in range(self.num_encoder_layers):
            ...
            skips.append(x)
        # Depth recurrence: repeat the last 2 encoder layers once more
        # This gives 11 virtual layers from 9 physical layers (free capacity!)
        recur_start = max(0, self.num_encoder_layers - 2)
    ...
    def _add_ngram_bias(self, logits: mx.array, prev_tokens: mx.array,
                        prev2_tokens: mx.array | None = None,
                        prev3_tokens: mx.array | None = None,
                        prev4_tokens: mx.array | None = None) -> mx.array:
        """Add bigram + trigram + 4-gram + 5-gram log-probability biases."""
        if self._bigram_logprobs is not None:
            bigram_bias = self._bigram_logprobs[prev_tokens]
            logits = logits + self._bigram_weight * bigram_bias.astype(logits.dtype)
        if self._trigram_logprobs is not None and prev2_tokens is not None:
            hashed = (36313 * prev_tokens.astype(mx.int32) + 27191 * prev2_tokens.astype(mx.int32)) % self._trigram_buckets
            trigram_bias = self._trigram_logprobs[hashed]
            logits = logits + self._trigram_weight * trigram_bias.astype(logits.dtype)
        if self._fourgram_logprobs is not None and prev3_tokens is not None:
            hashed4 = (36313 * prev_tokens.astype(mx.int32) + 27191 * prev2_tokens.astype(mx.int32) + 51497 * prev3_tokens.astype(mx.int32)) % self._fourgram_buckets
            fourgram_bias = self._fourgram_logprobs[hashed4]
            logits = logits + self._fourgram_weight * fourgram_bias.astype(logits.dtype)
        if self._fivegram_logprobs is not None and prev4_tokens is not None:
            hashed5 = (36313 * prev_tokens.astype(mx.int32) + 27191 * prev2_tokens.astype(mx.int32) + 51497 * prev3_tokens.astype(mx.int32) + 13397 * prev4_tokens.astype(mx.int32)) % self._fivegram_buckets
            fivegram_bias = self._fivegram_logprobs[hashed5]
            logits = logits + self._fivegram_weight * fivegram_bias.astype(logits.dtype)
        return logits

    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        # Cross-entropy with n-gram logit biases + complementary weighting.
        # Complementary: weight = clamp(1 - alpha * p_bigram(y|x), min=0.1)
        # When bigram already predicts the token, weight is LOW → model focuses on hard cases.
        x = self(input_ids).reshape(-1, self.tok_emb.weight.shape[1])
        y = target_ids.reshape(-1)
        input_flat = input_ids.reshape(-1)
        prev_tokens = input_flat
        prev2_tokens = mx.concatenate([mx.zeros((1,), dtype=mx.int32), input_flat[:-1]])
        prev3_tokens = mx.concatenate([mx.zeros((2,), dtype=mx.int32), input_flat[:-2]])
        prev4_tokens = mx.concatenate([mx.zeros((3,), dtype=mx.int32), input_flat[:-3]])

        # Compute complementary weights from bigram probabilities
        comp_weights = None
        if self._bigram_logprobs is not None:
            alpha = 0.5
            # p_bigram(y|prev) for each position
            bigram_lp = self._bigram_logprobs[prev_tokens]  # [N, vocab]
            # Gather the probability of the actual target token
            p_target = mx.exp(bigram_lp[mx.arange(y.shape[0]), y])  # [N]
            comp_weights = mx.maximum(1.0 - alpha * p_target, 0.1)

        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            logits_proj = x @ self.tok_emb.weight.astype(x.dtype).T
            logits = self._add_ngram_bias(self.softcap(logits_proj), prev_tokens, prev2_tokens, prev3_tokens, prev4_tokens)
            per_token = nn.losses.cross_entropy(logits.astype(mx.float32), y, reduction="none")
            if comp_weights is not None:
                return mx.sum(per_token * comp_weights) / mx.sum(comp_weights)
        ...
        for s in range(0, n, self.logit_chunk_tokens):
            ...
            logits = self._add_ngram_bias(self.softcap(logits_proj), prev_tokens[s:e], prev2_tokens[s:e], prev3_tokens[s:e], prev4_tokens[s:e])
            per_token = nn.losses.cross_entropy(logits.astype(mx.float32), y[s:e], reduction="none")
            if comp_weights is not None:
                w = comp_weights[s:e]
                loss_sum = loss_sum + mx.sum(per_token * w)
                weight_sum = weight_sum + mx.sum(w)
            else:
                loss_sum = loss_sum + mx.sum(per_token)
...
class Muon:
    ...
    def step(self, params: dict[str, mx.array], grads: dict[str, mx.array], step: int, lr_mul: float) -> dict[str, mx.array]:
        ...
        for k in self.keys:
            p = params[k]
            # Decoupled weight decay: shrink weights before gradient step
            if wd > 0:
                p = p * (1.0 - wd * lr)
            g = grads[k]
...
def main() -> None:
    ...
    # ==============================================================================
    # BIGRAM LOGPROB BIAS (precomputed from training data)
    # ==============================================================================
    bigram_path = Path("data/bigram_logprobs.npy")
    bigram_logprobs = None
    bigram_weight = float(os.environ.get("BIGRAM_WEIGHT", 0.3))
    if bigram_path.exists():
        bigram_logprobs_np = np.load(str(bigram_path))
        bigram_logprobs = mx.array(bigram_logprobs_np, dtype=mx.float32)
        log(f"bigram_logprob_bias:enabled weight:{bigram_weight} shape:{bigram_logprobs_np.shape}")
    else:
        log(f"bigram_logprob_bias:disabled (no file at {bigram_path})")

    # ==============================================================================
    # TRIGRAM LOGPROB BIAS (precomputed from training data, hash-based)
    # ==============================================================================
    trigram_path = Path("data/trigram_logprobs_8k.npy")
    trigram_logprobs = None
    trigram_weight = float(os.environ.get("TRIGRAM_WEIGHT", 0.15))
    trigram_buckets = 65536
    if trigram_path.exists():
        trigram_logprobs_np = np.load(str(trigram_path))
        trigram_logprobs = mx.array(trigram_logprobs_np, dtype=mx.float32)
        trigram_buckets = trigram_logprobs_np.shape[0]
        log(f"trigram_logprob_bias:enabled weight:{trigram_weight} shape:{trigram_logprobs_np.shape} buckets:{trigram_buckets}")
    else:
        log(f"trigram_logprob_bias:disabled (no file at {trigram_path})")

    # ==============================================================================
    # 4-GRAM LOGPROB BIAS
    # ==============================================================================
    fourgram_path = Path("data/fourgram_logprobs_8k.npy")
    fourgram_logprobs = None
    fourgram_weight = float(os.environ.get("FOURGRAM_WEIGHT", 0.1))
    fourgram_buckets = 65536
    if fourgram_path.exists():
        fourgram_logprobs_np = np.load(str(fourgram_path))
        fourgram_logprobs = mx.array(fourgram_logprobs_np, dtype=mx.float32)
        fourgram_buckets = fourgram_logprobs_np.shape[0]
        log(f"fourgram_logprob_bias:enabled weight:{fourgram_weight} shape:{fourgram_logprobs_np.shape} buckets:{fourgram_buckets}")
    else:
        log(f"fourgram_logprob_bias:disabled (no file at {fourgram_path})")

    # ==============================================================================
    # 5-GRAM LOGPROB BIAS
    # ==============================================================================
    fivegram_path = Path("data/fivegram_logprobs_8k.npy")
    fivegram_logprobs = None
    fivegram_weight = float(os.environ.get("FIVEGRAM_WEIGHT", 0.08))
    fivegram_buckets = 65536
    if fivegram_path.exists():
        fivegram_logprobs_np = np.load(str(fivegram_path))
        fivegram_logprobs = mx.array(fivegram_logprobs_np, dtype=mx.float32)
        fivegram_buckets = fivegram_logprobs_np.shape[0]
        log(f"fivegram_logprob_bias:enabled weight:{fivegram_weight} shape:{fivegram_logprobs_np.shape} buckets:{fivegram_buckets}")
    else:
        log(f"fivegram_logprob_bias:disabled (no file at {fivegram_path})")
    ...
    model = GPT(
        ...
        fourgram_logprobs=fourgram_logprobs,
        fourgram_weight=fourgram_weight,
        fourgram_buckets=fourgram_buckets,
        fivegram_logprobs=fivegram_logprobs,
        fivegram_weight=fivegram_weight,
        fivegram_buckets=fivegram_buckets,
    )
