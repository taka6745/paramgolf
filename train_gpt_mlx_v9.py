...
class GPT(nn.Module):
    ...
    def _add_ngram_bias(self, logits: mx.array, prev_tokens: mx.array, prev2_tokens: mx.array | None = None) -> mx.array:
        """Add bigram + trigram log-probability biases to model logits."""
        ...
        if self._trigram_logprobs is not None and prev2_tokens is not None:
            # Hash(prev2, prev1) -> bucket index, then lookup trigram logprobs
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        # Cross-entropy with bigram + trigram logit biases.
        x = self(input_ids).reshape(-1, self.tok_emb.weight.shape[1])
        y = target_ids.reshape(-1)
        # Previous tokens for n-gram lookup
        prev_tokens = input_ids.reshape(-1)  # prev1 (t-1)
        # prev2 tokens (t-2): shift by one more, pad first position with 0
        input_flat = input_ids.reshape(-1)
        prev2_tokens = mx.concatenate([mx.zeros((1,), dtype=mx.int32), input_flat[:-1]])
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            logits_proj = x @ self.tok_emb.weight.astype(x.dtype).T
            logits = self._add_ngram_bias(self.softcap(logits_proj), prev_tokens, prev2_tokens)
        ...
        for s in range(0, n, self.logit_chunk_tokens):
            ...
            logits = self._add_ngram_bias(self.softcap(logits_proj), prev_tokens[s:e], prev2_tokens[s:e])
