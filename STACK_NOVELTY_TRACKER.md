...
| STACK_GATED_NORM_PCT_LEGAL_TTT | - | gated_attention + L05_NORM_PCT_DROPOUT + LEGAL_TTT (3-way MINIMAL champion + new) | **yes (world-novel combination)** | **in-flight Pod B (started 1652Z, lands ~1725Z)** | hypothesis: ADD ONLY ONE world-novel (NORM_PCT) to MINIMAL champion stack; if composes, target 1.366-1.370 BEAR-FRUIT THRESHOLD | pending | B | 20260408T1650Z |
| STACK_LEGAL_TTT_NGRAM_BACKOFF | - | gated_attention + LEGAL_TTT + NGRAM_BACKOFF (cross-layer L04+L09) | **yes (world-novel combination)** | **C90 1720Z queued** | hypothesis: cross-LAYER stack (L04 model-side LEGAL_TTT champion + L09 n-gram-side BACKOFF bias) is ORTHOGONAL — different mechanisms (eval-time SGD vs n-gram logit residuals). Should compose better than ingredient stacking which we proved hurts. Both confirmed wins; n=2 for Pod J | pending | J | 20260408T1720Z |
...
| G2_gpu_idle_streak | 20260408T1717Z | 0 idle streaks (all 8 pods 85-100%); B STACK_GATED_NORM_PCT_LEGAL_TTT_seed42 25min in (lands ~1725Z); 3 NIGHT_MODE shots in flight | 0 streaks >5s util<80% | PASS | 0 |
